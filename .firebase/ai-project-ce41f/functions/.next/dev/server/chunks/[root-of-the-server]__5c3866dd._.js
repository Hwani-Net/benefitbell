...
async function POST(req) {
    try {
        ...
        const benefit = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$data$2f$benefits$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["BENEFITS"].find((b)=>b.id === benefitId);
        if (!benefit) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const isKo = lang === 'ko';
        const benefitInfo = isKo ? `혜택명: ${benefit.title}
설명: ${benefit.description}
지원금액: ${benefit.amount}
대상연령: ${benefit.targetAge ?? '제한 없음'}
소득기준: ${benefit.incomeLevel ?? '제한 없음'}
주관부처: ${benefit.ministry}
기존 자격조건: ${benefit.eligibilityChecks.map((e)=>e.label).join(', ')}` : `Benefit: ${benefit.titleEn}
...
Eligibility: ${benefit.eligibilityChecks.map((e)=>e.labelEn).join(', ')}`;
        const prompt = isKo ? `${benefitInfo}

위 혜택의 자격 조건을 확인하는 Yes/No 질문을 5개 만들어주세요.
질문은 사용자가 쉽게 이해할 수 있어야 합니다.
형식:
{"questions": ["질문1", "질문2", "질문3", "질문4", "질문5"]}` : `${benefitInfo}
...
async function PUT(req) {
    try {
        ...
        const isKo = lang === 'ko';
        const qa = questions.map((q, i)=>`${q}: ${answers[i] ? isKo ? '예' : 'Yes' : isKo ? '아니오' : 'No'}`).join('\n');
        const prompt = isKo ? `혜택명: ${benefit.title}
사용자 답변:
${qa}
...
형식:
{"verdict": "likely|partial|unlikely", "reason": "2~3문장 설명", "tips": "추가 안내 또는 다음 단계 (선택적)"}` : `Benefit: ${benefit.titleEn}
User answers:
${qa}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5c3866dd._.js.map
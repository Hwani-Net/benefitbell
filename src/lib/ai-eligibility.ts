...
export interface EligibilityResult {
  ...
  // Quick Win #1: 자연어 사유 설명 (PRD §6-5). Fallback 시 빈 문자열.
...
export async function assessBatch(
  ...
  for (let i = 0; i < uncached.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.warn(
        "[ai-eligibility] OpenRouter batch failed, using fallback:",
        err,
      );
...
function keywordFallback(
  ...
  // Age matching
  if (age < 35 && (benefit.category === "youth" || text.includes("청년")))
    score += 20;
  if (
    age >= 60 &&
    (benefit.category === "senior" ||
      text.includes("노인") ||
      text.includes("고령"))
  )
    score += 20;
  if (
    age >= 40 &&
    age < 60 &&
    (benefit.category === "middle-aged" || text.includes("중장년"))
  )
    score += 20;
  ...
  if (profile.region) {
    ...
    if (
      parts[0] &&
      text.includes(parts[0].replace("광역시", "").replace("특별시", ""))
    )
      score += 10;
  ...
  if (
    ...
    (benefit.category === "small-biz" || text.includes("소상공인"))
  )
    score += 15;
  if (
    profile.employmentStatus === "student" &&
    (benefit.category === "education" || text.includes("학생"))
  )
    score += 15;
  ...
  // Income
  if (
    profile.incomePercent <= 50 &&
    (benefit.category === "basic-living" || benefit.category === "near-poverty")
  )
    score += 15;

...
function classifyCategory(
  ...
  for (const [category, rule] of Object.entries(CATEGORY_RULES) as [
    BenefitCategory,
    CategoryRule,
  ][]) {
    let score = 0;
...
/**
 * Normalize various date formats to "YYYY.MM.DD" for calendar matching.
...
export function calculateDDay(endDateStr: string | null | undefined): number {
  ...
  if (
    endDateStr.includes("상시") ||
    endDateStr.includes("연중") ||
    endDateStr.includes("별도")
  ) {
    return 365;
  }
 ...
 */
export async function fetchWelfareList(
  pageNo = 1,
  numOfRows = 500,
  ...
  try {
    ...
    if (resultCode !== "0" && resultCode !== "00") {
      console.error(
        `[welfare-api] API error: ${resultCode} - ${xmlGet(text, "resultMessage")}`,
      );
      return [];
    }
...
export async function fetchWelfareDetail(
  servId: string,
): Promise<WelfareDetailItem | null> {
  const serviceKey = process.env.DATA_GO_KR_SERVICE_KEY;
 ...
 */
export function transformListItemToBenefit(
  ...
  return {
    ...
    applicationStart: normalizeDateStr(item.applicationStartDate),
    applicationEnd: normalizeDateStr(item.applicationEndDate),
    dDay: calculateDDay(item.applicationEndDate || ""),
    status: getStatus(
      item.applicationStartDate || null,
      item.applicationEndDate || null,
    ),
...
export function transformDetailToBenefit(item: WelfareDetailItem): Benefit {
  const category = classifyCategory(
    item.servNm,
    `${item.trgterIndvdl} ${item.alwServCn}`,
  );
  const labels = CATEGORY_LABELS[category];
...
export async function fetchLocalGovWelfareList(): Promise<WelfareListItem[]> {
  ...
  try {
    ...
    if (firstRes.status === 403 || firstRes.status === 401) {
      console.warn(
        "[welfare-api] 지자체 API 미인가 — data.go.kr에서 활용신청 필요",
      );
      return [];
    }
    if (!firstRes.ok) return [];
...
export async function fetchSubsidy24List(): Promise<WelfareListItem[]> {
  ...
  try {
    ...
    if (res.status === 403 || res.status === 401 || res.status === 500) {
      console.warn(
        "[welfare-api] 보조금24 API 미인가/에러 — data.go.kr에서 활용신청 필요",
      );
      return [];
    }
    if (!res.ok) return [];
...
export async function fetchBizinfoList(): Promise<WelfareListItem[]> {
  ...
  try {
    ...
    return items.map((item: Record<string, string>) => ({
      ...
      servDtlLink:
        item.rceptEngnHmpgUrl ||
        (item.pblancUrl ? `https://www.bizinfo.go.kr${item.pblancUrl}` : ""),
      inqNum: Number(item.inqireCo) || 0,
      svcfrstRegTs: item.creatPnttm || "",
      lastModYmd: item.creatPnttm?.substring(0, 10) || "",
      applicationEndDate: item.rcptEndYmd
        ? item.rcptEndYmd.replace(/(\d{4})(\d{2})(\d{2})/, "$1.$2.$3")
        : "",
      applicationStartDate: item.rcptBgnYmd
        ? item.rcptBgnYmd.replace(/(\d{4})(\d{2})(\d{2})/, "$1.$2.$3")
        : "",
    }));
  } catch (err) {
...
export async function fetchKStartupList(): Promise<WelfareListItem[]> {
  ...
  try {
    ...
    if (res.status === 403 || res.status === 401) {
      console.warn(
        "[welfare-api] K-Startup API 미인가 — data.go.kr 활용신청 필요",
      );
      return [];
    }
    if (!res.ok) return [];
    ...
    for (const block of itemBlocks) {
      ...
      items.push({
        ...
        servDtlLink: detlUrl.startsWith("http")
          ? detlUrl
          : detlUrl
            ? `https://${detlUrl}`
            : "",
        ...
        svcfrstRegTs: "",
        lastModYmd:
          getCol("pbanc_rcpt_bgng_dt")?.replace(
            /(\d{4})(\d{2})(\d{2})/,
            "$1-$2-$3",
          ) || "",
        applicationEndDate: (() => {
          const ed = getCol("pbanc_rcpt_ed_dt");
          return ed ? ed.replace(/(\d{4})(\d{2})(\d{2})/, "$1.$2.$3") : "";
        })(),
...
function getPrivateWelfareList(): WelfareListItem[] {
  ...
  return (privateWelfareData as PrivateWelfareItem[])
    ...
    .map((item) => ({
      ...
      applicationEndDate: item.endDate || "",
      applicationStartDate: item.startDate || "",
    }));
}
...
export async function fetchAllWelfareSources(): Promise<WelfareListItem[]> {
  ...
  const benefits = merged.filter(
    (item) => !isInstitutionalProgram(item.servNm),
  );

...
export function computeRuleScore(
  ...
  if (benefit.incomeLevel) {
    const incomeResult = matchIncomeLevel(
      benefit.incomeLevel,
      profile.incomePercent,
    );
  ...
  // ── 5. 고용상태 키워드 매칭 ──────────────────────
  const empMatch = matchEmployment(
  ...
  // ── 6. 주거형태 매칭 ────────────────────────────
  if (
    profile.housingType === "monthly" &&
    (text.includes("월세") || text.includes("임차"))
  ) {
    score += 10;
    matchReasons.push("월세 거주자 대상");
  }
  if (
    profile.housingType === "deposit" &&
    (text.includes("전세") || text.includes("임차"))
  ) {
    score += 10;
  ...
  const effectiveHasChildren =
    profile.maritalStatus !== "single" && profile.hasChildren;
  const effectiveChildrenCount =
    profile.maritalStatus !== "single" ? profile.childrenCount || 0 : 0;
  const effectiveChildrenAgeGroup =
    profile.maritalStatus !== "single" ? profile.childrenAgeGroup || [] : [];
  ...
  if (benefit.eligibilityChecks?.length > 0) {
    const checkMatch = matchEligibilityChecks(
      benefit.eligibilityChecks,
      profile,
      age,
    );
  ...
  const hasBasicLivingContext = basicLivingKeywords.some((k) =>
    text.includes(k),
  );
  ...
  // ── 12. 의료급여 수급 매칭 ─────────────────────────
  if (text.includes("의료급여") && profile.isMedicalAidRecipient) {
    score += 15;
  ...
  if (hasBizContext && profile.isBusinessOwner) {
    ...
    // 매출 규모 매칭
    if (
      (text.includes("소상공인") || text.includes("소기업")) &&
  ...
  if (effectiveChildrenCount > 0 && effectiveChildrenAgeGroup.length > 0) {
    if (
      effectiveChildrenAgeGroup.includes("infant") &&
      "영유아,어린이집,보육,유치원,아동수당"
        .split(",")
    ...
    if (
      effectiveChildrenAgeGroup.includes("elementary") &&
      "초등,방과후,돌봄".split(",").some((k) => text.includes(k))
    ) {
      score += 10;
    }
    if (
      effectiveChildrenAgeGroup.includes("teen") &&
...
function matchIncomeLevel(
  incomeLevel: string,
  userIncomePercent: number,
): { score: number; reason: string } {
  // Examples: "중위소득 50% 이하", "기준중위소득 120%", "소득기준 없음"
  if (
    incomeLevel.includes("없음") ||
    incomeLevel.includes("무관") ||
    incomeLevel.includes("제한없")
  ) {
    return { score: 5, reason: "" };
  }
...
function matchEligibilityChecks(
  ...
  for (const check of checks) {
    ...
    if (label.includes("의료급여") && profile.isMedicalAidRecipient) bonus += 5;
    if (label.includes("임신") && profile.isPregnant) bonus += 5;
    // 사업자
    if (
      (label.includes("사업자") || label.includes("소상공인")) &&
...
export function getFilteredBenefits(
  ...
  likely: FilteredBenefit[];
  partial: FilteredBenefit[];
  unlikely: FilteredBenefit[];
} {
  ...
  for (const benefit of benefits) {
    const { score, verdict, summary } = computeRuleScore(
      benefit,
      profile,
      isVerified,
    );
...
export function getPersonalizedBenefits(
  ...
  return benefits
    .map((b) => ({
      benefit: b,
      score: computeRuleScore(b, profile, isVerified).score,
    }))

...
export const translations = {
  ko: {
    ...
    urgentBannerSub: (name: string, extra: number) =>
      `${name} 외 ${extra}건 — 놓치지 마세요`,
    ...
    supportDesc:
      "이 앱이 도움이 되셨나요? 개발자에게 커피 한 잔을 선물해주세요!",
  ...
  en: {
    ...
    urgentBannerSub: (name: string, extra: number) =>
      `${name} and ${extra} more — don't miss out`,
...
interface AppContextType {
  ...
  setKakaoUser: (
    u: { nickname: string; profile_image?: string } | null,
  ) => void;
  benefits: Benefit[];
  ...
  useEffect(() => {
    fetch("/api/benefits")
      ...
      .then((json) => {
        if (json.data)
          setBenefits(
            (json.data as Benefit[]).filter(
              (b: Benefit) => b.status !== "closed",
            ),
          );
      })
    ...
    if (kakaoUser?.id) {
      const timer = setTimeout(async () => {
        try {
          ...
          console.warn("[context] bookmark sync failed:", e);
        }
      }, 1000); // 1초 디바운스

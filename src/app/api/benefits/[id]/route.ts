...
export async function GET(
    ...
    if (cacheDoc.exists) {
      ...
      if (age < CACHE_TTL_MS) {
        ...
        if (
          ...
          isInstitutionalProgram(detail.title)
        ) {
          return NextResponse.json(
            {
              success: false,
  ...
  let notFound = false; // PP-004: data.go.kr이 존재하지 않는 ID에 "NO DATA FOUND" 반환 시 404 즉시 반환
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      // PP-004: data.go.kr이 존재하지 않는 servId에 "NO DATA FOUND" 메시지 반환
      // (resultCode가 비어있어도 본문에 메시지만 있는 경우 — 재시도 무의미, 404 즉시 반환)
      if (/NO\s*DATA\s*FOUND/i.test(xml)) {
        notFound = true;
        break;
      }
      ...
      if (resultCode && resultCode !== "0" && resultCode !== "00") {
        const msg =
          getXmlValue(xml, "resultMessage") || `resultCode: ${resultCode}`;
      ...
      Object.assign(detail, {
        ...
        // Inquiry contacts
        contacts: getXmlValues(xml, "inqplCtadrList", "servSeDetailNm").map(
          (name, i) => ({
            name,
            address:
              getXmlValues(xml, "inqplCtadrList", "servSeDetailLink")[i] || "",
          }),
        ),
        // Required documents (name + download link)
        requiredDocs: getXmlValues(xml, "basfrmList", "servSeDetailNm").map(
          (name, i) => ({
            name,
        ...
        // Inquiry homepage
        homepages: getXmlValues(xml, "inqplHmpgReldList", "servSeDetailNm").map(
          (name, i) => ({
            name,
            url:
              getXmlValues(xml, "inqplHmpgReldList", "servSeDetailLink")[i] ||
              "",
          }),
        ),
      });
  ...
  // PP-004: data.go.kr "NO DATA FOUND" → 404 명시 반환 (P0)
  if (notFound || /NO\s*DATA\s*FOUND/i.test(lastError)) {
    return NextResponse.json(

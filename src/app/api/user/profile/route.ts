...
async function verifyKakaoIdFromAuth(
  ...
  try {
    ...
    const tokenKakaoId = decoded.uid.startsWith("kakao_")
      ? decoded.uid.slice(6)
      : decoded.uid;
    if (String(tokenKakaoId) !== String(requestedKakaoId)) {
      return {
        ok: false,
        status: 403,
        error: "다른 사용자의 프로필에 접근할 수 없습니다.",
      };
    }
    return { ok: true };
  } catch (err) {
    console.error("[user/profile] verifyIdToken failed:", err);
    return { ok: false, status: 403, error: "유효하지 않은 인증 토큰입니다." };
  }
}
...
export async function GET(req: Request) {
  try {
    ...
    const authCheck = await verifyKakaoIdFromAuth(req, kakaoId);
    if (!authCheck.ok) {
...
export async function POST(req: Request) {
  try {
    ...
    const authCheck = await verifyKakaoIdFromAuth(req, String(kakaoId));
    if (!authCheck.ok) {
      return NextResponse.json(
        { error: authCheck.error },
        { status: authCheck.status },
    ...
    if (body.incomePercent !== undefined)
      updateData.incomePercent = body.incomePercent;
    if (body.housingType !== undefined)
      updateData.housingType = body.housingType;
    if (body.employmentStatus !== undefined)
      updateData.employmentStatus = body.employmentStatus;
    if (body.specialStatus !== undefined)
      updateData.specialStatus = body.specialStatus;
    // Step 2: 가족
    ...
    if (body.childrenCount !== undefined)
      updateData.childrenCount = body.childrenCount;
    if (body.childrenAgeGroup !== undefined)
      updateData.childrenAgeGroup = body.childrenAgeGroup;
    ...
    if (body.businessAge !== undefined)
      updateData.businessAge = body.businessAge;
    ...
    if (body.industryType !== undefined)
      updateData.industryType = body.industryType;
    // 시스템
    if (body.kakaoAlerts !== undefined)
      updateData.kakaoAlerts = body.kakaoAlerts;

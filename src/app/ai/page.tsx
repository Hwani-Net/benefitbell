...
function FilterCard({
  ...
  const isKo = lang === "ko";
  const scoreColor =
    benefit.verdict === "likely"
      ? "#15803d"
      : benefit.verdict === "partial"
        ? "#92400e"
        : "#dc2626";
  const scoreBg =
    benefit.verdict === "likely"
  ...
  return (
    <Link href={`/detail/${benefit.id}`} className={styles.filterCard}>
      <div className={styles.filterCardInfo}>
        <h3 className={styles.filterCardTitle}>
          {bText(benefit, "title", lang)}
        </h3>
        <p className={styles.filterCardAmount}>
          {bText(benefit, "amount", lang)}
        </p>
        <div className={styles.filterCardMeta}>
          <span className="badge badge-coral-soft text-xs">
            {bText(benefit, "categoryLabel", lang)}
          </span>
  ...
  async function handleChatSubmit() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (err.error === "AI_KEY_INVALID") {
          throw new Error(
            isKo
              ? "AI 서비스를 일시적으로 이용할 수 없습니다."
            ...
            {!hasProfile ? (
              ...
              <div className={styles.profileCta}>
                ...
                <h2 className={styles.profileCtaTitle}>
                  {isKo
                    ? "30초 프로필 입력으로\nAI 맞춤 혜택을 발견하세요"
                    : "Enter your profile in 30 seconds\nto discover matching benefits"}
                </h2>
                ...
                {kakaoUser ? (
                  ...
                  <a
                    href="/api/auth/kakao"
                    className={styles.profileCtaBtn}
                    style={{
                      background: "#FEE500",
                      color: "#000",
                      border: "none",
                  ...
                  <div className={styles.statsRow}>
                    ...
                    <div className={styles.statCard}>
                      <p
                        ...
                        style={{ color: "#92400e" }}
                      >
                        {filtered.partial.length}
                      </p>
                      <p className={styles.statLabel}>
                        {isKo ? "⚠️ 확인 필요" : "⚠️ Check"}
                      </p>
                    </div>
                    <div className={styles.statCard}>
                      <p
                  ...
                  <>
                    <div className={styles.groupHeader}>
                      ...
                      <h3 className={styles.groupTitle}>
                        {isKo ? "수령 가능성 높음" : "Likely Eligible"}
                      </h3>
                    ...
                    <div className={styles.benefitList}>
                      {filtered.likely.map((b) => (
                        <FilterCard key={b.id} benefit={b} lang={lang} />
                      ))}
                    </div>
                  </>
                )}
                ...
                {filtered && filtered.unlikely.length > 0 && (
                  <>
                    ...
                    >
                      {showUnlikely
                        ? isKo
                          ? `▲ 해당 안됨 ${filtered.unlikely.length}건 접기`
                          : `▲ Hide ${filtered.unlikely.length} unlikely`
                        : isKo
                          ? `▼ 해당 안됨 ${filtered.unlikely.length}건 보기`
                          : `▼ Show ${filtered.unlikely.length} unlikely`}
                    </button>
                    {showUnlikely && (
                      <div
                        className={styles.benefitList}
                        style={{ marginTop: 12, opacity: 0.65 }}
                      >
                        {filtered.unlikely.map((b) => (
                          <FilterCard key={b.id} benefit={b} lang={lang} />
                        ))}
                      </div>
                    )}
                ...
                {filtered &&
                  ...
                  filtered.partial.length === 0 && (
                    <div style={{ textAlign: "center", padding: "40px 20px" }}>
                      ...
                      >
                        {isKo
                          ? "현재 프로필로 매칭되는 혜택이 없습니다"
                          : "No matching benefits for your profile"}
                      </p>
                      <p
                        style={{
                          fontSize: 13,
                      ...
                      >
                        {isKo
                          ? "프로필 정보를 추가하면 더 정확한 결과를 받을 수 있어요"
                          : "Add more profile info for better results"}
                      </p>
                      {kakaoUser ? (
                        <Link
                          href="/profile"
          ...
          <>
              ...
              >
                ...
                {!userProfile?.isPremium && (
                  <span className="badge badge-coral-soft">
                    {isKo
                      ? `무료 ${10 - usageCount}회 남음`
                      : `${10 - usageCount} free left`}
                  </span>
                )}
                <button
            ...
            {!chatResult && !chatLoading && (
              <div className={styles.examples}>
                ...
                <div className={styles.exampleList}>
                  {(isKo ? EXAMPLE_PROMPTS_KO : EXAMPLE_PROMPTS_EN).map(
                    (ex, i) => (
                      <button
                        key={i}
            ...
            {chatLoading && (
              <div className={styles.loadingState}>
                ...
                <p>
                  {isKo
                    ? "AI가 혜택 데이터베이스를 분석하고 있어요..."
                    : "AI is analyzing the benefits database..."}
                </p>
              </div>
            )}
                      ...
                      <div key={id} className={styles.benefitCard}>
                        <div className={styles.cardTop}>
                          ...
                          {benefit?.dDay !== undefined &&
                            ...
                            benefit.dDay <= 30 && (
                              <span
                                ...
                                style={{
                                  fontSize: 11,
                                  color: getDDayColor(benefit.dDay),
                                }}
                              >
                                {getDDayText(benefit.dDay, isKo ? "ko" : "en")}
                              </span>
                          ...
                          <button
                            ...
                            onClick={() =>
                              benefit &&
                              handleShare(id, bText(benefit, "title", lang))
                            }
                        ...
                        >
                          <h3 className={styles.cardTitle}>
                            {benefit ? bText(benefit, "title", lang) : id}
                          </h3>
                          {benefit && (

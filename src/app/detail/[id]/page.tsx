...
export default function DetailPage({
  ...
  const foundBenefit = useMemo(
    () => allBenefits.find((b) => b.id === id) ?? null,
    [allBenefits, id],
  );
  ...
  useEffect(() => {
    ...
    async function fetchDetail(benefitId: string) {
      ...
      if (
        ...
        benefitId.startsWith("SUB-") ||
        benefitId.startsWith("BIZ-") ||
        benefitId.startsWith("KSU-") ||
        ...
        if (json.success && json.data) {
          ...
          if (foundBenefit && (json.data.applyBgnDt || json.data.applyEndDt)) {
            setEnrichedBenefit({
              ...foundBenefit,
              applicationStart:
                json.data.applyBgnDt ?? foundBenefit.applicationStart,
              applicationEnd:
                json.data.applyEndDt ?? foundBenefit.applicationEnd,
            });
          }
        }
      } catch {
  ...
  const category =
    lang === "ko" ? benefit.categoryLabel : benefit.categoryLabelEn;
  const ministry =
    apiDetail?.ministry ||
    (lang === "ko" ? benefit.ministry : benefit.ministryEn);
  const steps = benefit.steps.map((s) =>
    lang === "ko"
      ? { title: s.title, desc: s.desc }
      : { title: s.titleEn, desc: s.descEn },
  );

  // 필요 서류: API 데이터 우선, 없으면 benefit 데이터, 둘 다 없으면 []
  const benefitDocs: Array<string | { name: string; link: string }> =
    lang === "ko" ? (benefit.documents ?? []) : (benefit.documentsEn ?? []);
  const docs: Array<string | { name: string; link: string }> = apiDetail
    ?.requiredDocs?.length
    ? apiDetail.requiredDocs
    : benefitDocs;
  ...
  const appStart =
    formatDate(benefit.applicationStart) ||
    formatDate(apiDetail?.year ? `${apiDetail.year}0101` : "");
          ...
          <div className={styles.infoGrid}>
            ...
            <div className={styles.infoItem}>
              ...
              <div>
                ...
                <p className={styles.infoValue}>
                  {apiDetail?.supportType || t.monthlyTransfer}
                </p>
              </div>
            </div>
          ...
          <section className="section">
            <div className={styles.eligCard}>
              <div className={styles.eligHeader}>
                ...
                <span className={styles.eligCount}>
                  {t.eligibilityCheck(fulfilled, checks.length)}
                </span>
              </div>
              ...
              <div className={styles.checkList}>
                {checks.map((c, i) => (
                  <div key={i} className={styles.checkItem}>
                    <span
                      className={c.pass ? styles.checkPass : styles.checkFail}
                    >
                      {c.pass ? "✅" : "⚠️"}
                    </span>
                    ...
                    <span
                      className={`${styles.checkBadge} ${c.pass ? styles.passBadge : styles.failBadge}`}
                    >
        ...
        >
          <div
            style={{
              ...
              width: `${progressPct}%`,
              background:
                checkedCount === total ? "#10b981" : "var(--primary, #7f13ec)",
              borderRadius: 3,
        ...
        {docsWithLinks.map((doc, i) => (
          ...
          >
            ...
            <button
              ...
              aria-label={
                checked[i] ? "서류 준비 완료 취소" : "서류 준비 완료 체크"
              }
              style={{
                width: 22,
                height: 22,

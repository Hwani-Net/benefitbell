...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const defaultMeta: Metadata = {
    ...
    description:
      "나에게 맞는 정부 지원금·복지 혜택을 혜택알리미에서 확인하세요.",
    openGraph: {
      title: "정부 지원금 상세 정보 | 혜택알리미",
      description:
        "나에게 맞는 정부 지원금·복지 혜택을 혜택알리미에서 확인하세요.",
      ...
      type: "article",
      images: [
        { url: `${BASE_URL}/opengraph-image`, width: 1200, height: 630 },
      ],
    },
    twitter: {
  ...
  try {
    ...
    if (detailRes.ok) {
      ...
      if (json.success && json.data) {
        ...
        const rawDesc = stripHtml(
          overview ||
            supportContent ||
            "나에게 맞는 정부 복지 혜택을 확인하세요.",
        );
        // OG description은 160자 이하로
        const ogDesc =
          rawDesc.length > 155 ? rawDesc.slice(0, 155) + "…" : rawDesc;
    ...
    if (listRes.ok) {
      ...
      const benefit = (listJson.data ?? []).find(
        (b: { id: string }) => b.id === id,
      );
      if (benefit) {

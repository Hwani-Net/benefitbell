    ...
    <>
      ...
      <main className="page-content">
            ...
            <p className={styles.subGreeting}>
              {loading
                ? lang === "ko"
                  ...
                  : "Loading benefits..."
                : t.urgentSubtitle(
                    urgentBenefits.filter((b) => b.dDay <= 14).length,
                  )}
            </p>
            {!kakaoUser && (
            ...
            {!loading && benefits.length > 0 && (
              ...
              >
                {lang === "ko"
                  ...
                  : `📊 ${benefits.length} welfare services`}
              </p>
            )}
            {!loading && apiError && (
              <p
              ...
              >
                {lang === "ko"
                  ? "⚠️ 데이터 업데이트 실패 — 잠시 후 다시 시도해주세요"
                  : "⚠️ Data update failed — please try again later"}
              </p>
            )}
          </div>
        ...
        <section>
          ...
          <div
            ref={dragScrollRef}
            className={`scroll-x ${styles.urgentScroll}`}
          >
            {loading ? (
              ...
              >
                {apiError
                  ? lang === "ko"
                    ? "⚠️ 데이터를 불러오지 못했습니다. 새로고침 해주세요."
                    : "⚠️ Failed to load data. Please refresh."
                  : lang === "ko"
                    ? "현재 마감 임박 혜택이 없습니다."
                    : "No expiring benefits at the moment."}
              </div>
            ) : (
                ...
                >
                  <div className={styles.urgentCardTop}>
                    ...
                    <span className={styles.ministry}>
                      {lang === "ko" ? benefit.ministry : benefit.ministryEn}
                    </span>
                  </div>
                  <h3 className={styles.urgentTitle}>
                    {bText(benefit, "title", lang)}
                  </h3>
                  <p className={styles.urgentAmount}>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {lang === "ko"
                      ? "나이·지역·소득 정보를 입력하세요 →"
                      : "Enter age, region, and income →"}
                  </div>
                </div>
                <span style={{ color: "rgba(255,255,255,0.9)", fontSize: 18 }}>
                  →
                </span>
              </div>
            </Link>
            ...
            <div className="section-header">
              <h2 className="section-title">
                {lang === "ko"
                  ...
                  : `✨ Personalized for ${kakaoUser.nickname}`}
                {allPersonalizedBenefits.length > 0 && (
                  ...
                  >
                    {lang === "ko"
                      ...
                      : `${allPersonalizedBenefits.length} total`}
                  </span>
                )}
              </h2>
              {allPersonalizedBenefits.length > 5 && (
                <Link href="/ai" className="section-link">
                  {lang === "ko" ? "전체 AI 분석 →" : "Full AI Analysis →"}
                </Link>
              )}
            </div>
                ...
                return (
                  ...
                  >
                    <div className={styles.benefitInfo}>
                      ...
                      <p className={styles.benefitAmount}>
                        {bText(benefit, "amount", lang)}
                      </p>
                      <div className={styles.benefitMeta}>
                        ...
                        {benefit.dDay <= 14 && benefit.dDay >= 0 && (
                          <span
                            className={`badge ${getDDayColor(benefit.dDay)} text-xs`}
                          >
                            {getDDayText(
                              benefit.dDay,
                              lang === "ko" ? "ko" : "en",
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {lang === "ko"
                      ? "AI가 나에게 딱 맞는 혜택을 찾아드려요"
                      : "AI finds benefits tailored just for you"}
                  </div>
                  <div
                    style={{
                      fontSize: 12,
                  ...
                  >
                    {lang === "ko"
                      ? "30초 프로필 입력 → 수령 가능성 % 즉시 확인"
                      : "Quick profile → See your eligibility % instantly"}
                  </div>
                </div>
                <span
                  style={{
          ...
          <div className={styles.benefitList}>
            {loading
              ? Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className={`${styles.benefitItem} shimmer`}
                    style={{ height: 72 }}
                  />
                ))
                ...
                >
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        handleShare(benefit.id, bText(benefit, "title", lang));
                      }}
                      aria-label={lang === "ko" ? "공유" : "Share"}
                    >
                      {sharedId === benefit.id ? "✅" : "📤"}

...
export default function CalendarPage() {
  ...
  const urgentBenefits = allBenefits
    .filter((b) => b.dDay >= 0 && b.dDay <= 7 && b.status === "open")
        ...
        <section className="section" style={{ paddingTop: 8 }}>
            ...
            <div className={styles.urgentBanner}>
              ...
              <div className={styles.urgentText}>
                ...
                <span>
                  {t.urgentBannerSub(
                    lang === "ko"
                      ? urgentBenefits[0].title
                      : urgentBenefits[0].titleEn,
                    urgentBenefits.length - 1,
                  )}
                </span>
              </div>
              <span className={`badge badge-red`}>
                D-{urgentBenefits[0].dDay}
              </span>
            </div>
          )}
          ...
          <div className={styles.calHeader}>
            ...
            <div className={styles.calTitle}>
              <span className={styles.calYear}>
                {t.calMonthTitle(year, month + 1)}
              </span>
          ...
          <div className={styles.dayHeader}>
            {dayNames.map((d: string, idx: number) => (
              <span
                key={d}
                className={`${styles.dayName} ${idx === 0 ? styles.sunday : ""}`}
              >
                {d}
              </span>
            ))}
                ...
                {selectedBenefits.map((b) => (
                  ...
                  >
                    <div className={styles.calCardLeft}>
                      ...
                      <h3 className={styles.calCardTitle}>
                        {bText(b, "title", lang)}
                      </h3>
                      <p className={styles.calCardAmount}>
                        {bText(b, "amount", lang)}
                      </p>
                    </div>
                    <div
          ...
          <div className={styles.monthSummary}>
            <h2 className="section-title mb-12">
              {t.monthlyDeadlineTitle(month + 1, monthlyDeadlines.length)}
            </h2>
                ...
                {monthlyDeadlines.map((b) => (
                  ...
                  >
                    <div className={styles.calCardLeft}>
                      ...
                      <p
                        className={`${styles.calCardDate}${b.dDay <= 3 ? " text-red-500" : b.dDay <= 7 ? " text-orange-400" : ""}`}
                      >
                        {t.deadlineLabel}: {b.applicationEnd}
                      </p>
                    </div>
                    <div
                      style={{
          ...
          {alwaysOpenBenefits.length > 0 && (
            <div className={styles.monthSummary}>
              <h2 className="section-title mb-12">
                {t.alwaysOpenTitle(alwaysOpenBenefits.length)}
              </h2>
              <div className={styles.benefitCards}>
                {alwaysOpenBenefits.slice(0, 10).map((b) => (
                  ...
                  >
                    <div className={styles.calCardLeft}>
                      <div className={styles.calCardMeta}>
                        <span className="badge badge-green-soft">
                          {bText(b, "categoryLabel", lang)}
                        </span>
                        <span className="badge badge-purple-soft">
                          {t.alwaysOpenBadge}
                        </span>
                      </div>
                      <h3 className={styles.calCardTitle}>
